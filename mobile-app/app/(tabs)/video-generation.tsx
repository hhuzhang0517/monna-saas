...
export default function VideoGenerationScreen() {
  ...
  const [loadedVideoIds, setLoadedVideoIds] = useState<Set<string>>(new Set()); // 已加载视频的ID
  const [isScrolling, setIsScrolling] = useState(false);

  const scrollYRef = useRef(0);
  const scrollViewHeightRef = useRef(0);
  const cardPositions = useRef<Map<string, { y: number; height: number }>>(new Map());
  const scrollEndTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const categories: CategoryLabel[] = [
    { id: 'effects', label: t('categories.effects', 'Effects') },
    { id: 'animation', label: t('categories.animation', 'Animation') },
    { id: 'fantasy', label: t('categories.fantasy', 'Fantasy') },
  ];
  ...
  // 检查哪些视频卡片完全可见并加载它们
  const loadVisibleVideos = useCallback(() => {
    const viewportTop = scrollYRef.current;
    const viewportBottom = scrollYRef.current + scrollViewHeightRef.current;
    const newLoadedIds = new Set<string>();

    cardPositions.current.forEach((pos, videoId) => {
      const cardTop = pos.y;
      ...
      // 只加载完全可见的视频（可见高度 = 卡片高度）
      if (visibleHeight === pos.height) {
        newLoadedIds.add(videoId);
      }
    });

    setLoadedVideoIds(newLoadedIds);
  }, []);

  // 处理滚动事件 - 滚动停止500ms后加载视频
  ...
  const handleScrollViewLayout = useCallback((event: any) => {
    ...
    // 初次加载时，延迟加载可见视频
    setTimeout(() => loadVisibleVideos(), 100);
  }, [loadVisibleVideos]);
  ...
  const handleGenerate = async (files: { video?: string; image?: string }) => {
    ...
    try {
      ...
      if (selectedTemplate.imageToVideo) {
        // 图片转视频：上传图片
        if (!files.image) {
          throw new Error(t('generate.pleaseUploadImage', 'Please upload an image'));
        }
        imageUrl = await uploadImage(files.image);
      } else if (selectedCategory === 'animation') {
        // 角色迁移：需要视频和图片
        if (!files.video || !files.image) {
          throw new Error(t('generate.uploadTwoImages', 'Please upload two images'));
        }
        ...
        // 特效和幻想：上传视频
        if (!files.video) {
          throw new Error(t('generate.pleaseUploadVideo', 'Please upload a video'));
        }
        videoUrl = await uploadVideo(files.video);
      }
  ...
  const uploadImage = async (imageUri: string): Promise<string> => {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) throw new Error(t('generate.pleaseLoginFirst', 'Please login first'));
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (attempts < maxAttempts) {
              ...
              throw new Error(t('generate.generationFailed', 'Generation failed'));
            }
          }
        ...
        <View style={styles.templatesGrid}>
          {filteredTemplates.map((template, index) => (
            <TemplateCard
              ...
              shouldLoadVideo={loadedVideoIds.has(template.id)}
              onPress={() => handleTemplateClick(template)}
              onLayout={(event) => {
                const { y, height } = event.nativeEvent.layout;
                cardPositions.current.set(template.id, { y, height });
                // 最后一个卡片布局完成后，初始加载可见视频
